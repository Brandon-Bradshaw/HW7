#region imports
import sys
from ThermoStateCalc_2 import Ui__frm_StateCalculator
...
class main_window(QWidget,Ui__frm_StateCalculator):
    ...
    def setUnits(self):
        ...
        SpecifiedProperty3 = self._cmb_Property1_2.currentText()
        SpecifiedProperty2 = self._cmb_Property2.currentText()
        SpecifiedProperty4 = self._cmb_Property2_2.currentText()

        #read numerical values for selected properties
        SP=[float(self._le_Property1.text()), float(self._le_Property2.text()), float(self._le_Property1_2.text())] # when adding, float(self._le_Property2_2.text()) we get an error
        ...
        #adding this, self._le_Property2_2.setText("{:0.3f}".format(SP[3])) gives an error
    ...
    def calculateProperties(self):
        ...
        if SP[2]=='p' or SP[3]=='p':
            oFlipped= SP[2] != 'p'
            SP1 = SP[2] if oFlipped else SP[3]
        #case 1:  pt or tp
            if SP3=='t':
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.p= f3 if not oFlipped else f4
                self.t= f4 if not oFlipped else f3
                self.getSatProps_p(self.p)
                self.tSat=round(self.tSat,3)  # I will compare at 3 three decimal places
                # compare T to TSat
                if self.t<self.tSat or self.t>self.tSat:
        ...
        #case 3 pu or up
                ...
                f4=float(self._le_Property2_2.text())
                self.p= f3 if not oFlipped else f4;
                self.u= f4 if not oFlipped else f3;
                self.getSatProps_p(self.p)
                # compare u to uf and ug
                if self.u<self.uf or self.u>self.ug:
                    self.region = "sub-cooled liquid" if self.u<self.uf else "super-heated vapor"
        ...
        #case 4 ph or hp
                ...
                f4=float(self._le_Property2_2.text())
                self.p= f3 if not oFlipped else f4;
                self.h= f4 if not oFlipped else f3;
                self.getSatProps_p(self.p)
                # compare h to hf and hg
                if self.h < self.hf or self.h > self.hg:
                    self.region = "sub-cooled liquid" if self.h < self.hf else "super-heated vapor"
        ...
        #case 5 ps or sp
                ...
                f4=float(self._le_Property2_2.text())
                self.p= f3 if not oFlipped else f4;
                self.s= f4 if not oFlipped else f3;
                self.getSatProps_p(self.p)
                # compare s to sf and sg
                if self.s < self.sf or self.s > self.sg:
                    self.region = "sub-cooled liquid" if self.s < self.sf else "super-heated vapor"
        ...
        #case 6 px or xp
                ...
                f4=float(self._le_Property2_2.text())
                self.p= f3 if not oFlipped else f4
                self.x= f4 if not oFlipped else f3
                self.getSatProps_p(self.p)
                self.t=self.tSat
                self.x=self.clamp(self.x,0.0,1.0)
                self.makeLabel_2Phase()
            ...
            oFlipped = SP[2] != 't'
            SP3 = SP[2] if oFlipped else SP[3]
        #case 7:  tv or vt
            if SP3=='v':
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.p= f3 if not oFlipped else f4
                self.v= f4 if not oFlipped else f3
                self.getSatProps_t(self.t)
                self.vf=round(self.vf,5)
                self.vg=round(self.vg,3)
                # compare v to vf and vg
        ...
        #case 8:  tu or ut
                ...
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.t= f3 if not oFlipped else f4
                self.u= f4 if not oFlipped else f3
                self.getSatProps_t(self.t)
                # compare u to uf and ug
                if self.u < self.uf or self.u > self.ug:
        ...
        #case 11:  tx or xt
            ...
            if SP3 == 'h':
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.v= f3 if not oFlipped else f4
                self.h= f4 if not oFlipped else f3
                def fn12(P):
                    # could be single phase or two-phase, but both v&h have to match at same x
                    self.getSatProps_p(P)
            ...
            if SP3 == 'u':
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.h= f3 if not oFlipped else f4
                self.u= f4 if not oFlipped else f3
                # use fsolve to fing P&T at this v & u
                def fn16(PT):
                    self.getSatProps_p(PT[0])
            ...
            if SP3 == 's':
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.u= f3 if not oFlipped else f4
                self.s= f4 if not oFlipped else f3
                def fn19(PT):
                    self.getSatProps_p(PT[0])
                    if self.between(self.s, self.sf, self.sg):
            ...
            if SP3 == 'x':
                f3=float(self._le_Property1_2.text())
                f4=float(self._le_Property2_2.text())
                self.s= f3 if not oFlipped else f4
                self.x= f4 if not oFlipped else f3
                self.region = "two-phase"
                def fn21(p):
                    self.getSatProps_p(p)
